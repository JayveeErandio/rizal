    ...
    <div className="space-y-12 animate-fade-in" id="women-view-container">
      <div className="text-center space-y-2">
        ...
        <p className="font-serif italic text-xs md:text-sm text-gold-800/80 max-w-2xl mx-auto">
          "Nine muses of varying cultures who illuminated his journeys, inspired his fictional heroines, and shared his dangerous exile."
        </p>
        <div className="w-20 h-0.5 bg-crimson-700 mx-auto mt-3" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start" id="women-grid">
        ...
        <div className="lg:col-span-5 space-y-4" id="women-selector-panel">
            ...
            {womenList.map((w) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute -bottom-4 -right-4 w-12 h-12 rounded-full border ${isSelected ? "border-gold-400/20" : "border-gold-600/10"} pointer-events-none`} />
          ...
          <div className="p-4 bg-gold-50/40 border border-dashed border-gold-600/20 rounded-xs text-[11px] text-amber-950/70 text-center italic leading-relaxed">
            "To love is to walk hand-in-hand in a dark alley, knowing that any light turned on might expose the danger of our convictions."
          </div>
        </div>

        {/* Right: Antique Picture Frame & Heart Correspondence Journal */}
        <div className="lg:col-span-7" id="women-cameo-viewer">
          <div className="parchment-card p-6 md:p-8 rounded-sm border-2 border-double border-gold-500 relative overflow-hidden flex flex-col justify-between min-h-[440px]" id="cameo-container">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start border-b border-gold-300/30 pb-5 mb-5" id="cameo-top">
              {/* Victorian Oval Cameo Profile Frame */}
              <div className="flex-shrink-0 w-24 h-32 rounded-full border-4 border-double border-gold-500 bg-gradient-to-b from-crimson-950 to-gold-950 flex items-center justify-center relative p-1 shadow-md overflow-hidden" id="cameo-oval-frame">
                <div className="absolute inset-0.5 rounded-full border border-gold-300/35 pointer-events-none" />
                {/* Initial inside frame */}
                <span className="font-display text-4xl font-extrabold text-gold-300 select-none tracking-widest opacity-80">
                  {selectedWoman.name.charAt(0)}
                </span>
              </div>

              {/* Identity Metadata */}
              <div className="space-y-1.5 text-center md:text-left flex-grow" id="cameo-identity">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1 font-serif text-xs text-amber-900/85">
                  <p><strong>Origins:</strong> {selectedWoman.nationality}</p>
                  <p><strong>Period:</strong> {selectedWoman.period}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4" id="cameo-body">
              ...
              <div className="p-3.5 bg-crimson-50/40 border border-dashed border-crimson-800/25 rounded-xs space-y-1 text-justify" id="cameo-legacy">
