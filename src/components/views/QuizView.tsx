...
import { historicalAudio } from "../../utils/historicalAudio";
...
export default function QuizView() {
  ...
  const [isMuted, setIsMuted] = useState<boolean>(historicalAudio.isMuted());

  // Initialize and randomize 5 questions from the pool
  useEffect(() => {
    startNewQuiz();

    // Start background music loop on mount
    historicalAudio.startMusic();

    return () => {
      // Pause music when navigating away
      historicalAudio.stopMusic();
    };
  }, []);
  ...
  const startNewQuiz = () => {
    ...
    // Resume music if unmuted
    historicalAudio.startMusic();
  };

  const handleToggleMute = () => {
    const muted = historicalAudio.toggleMute();
    setIsMuted(muted);
    if (!muted) {
      historicalAudio.startMusic();
    } else {
  ...
  const handleOptionSelect = (optionIdx: number) => {
    ...
    // User interaction: ensure audio context is active
    historicalAudio.startMusic();
  };
  ...
  const handleSubmitAnswer = () => {
    ...
    if (selectedOptionIdx === currentQuestion.correctAnswerIndex) {
      ...
      historicalAudio.playCorrect();
    } else {
      historicalAudio.playIncorrect();
    }
    setIsSubmitted(true);
  };
  ...
  const getEvaluation = (finalScore: number) => {
    if (finalScore === 5) {
      return {
        ...
        description:
          "Exquisite! Your intellect matches the highest scholars of Ateneo. You are fit to write the sequel to El Filibusterismo!",
      ...
      return {
        ...
        description:
          "Brilliant! You have a highly profound understanding of Rizal's historical logs and life achievements.",
      ...
      return {
        ...
        description:
          "A solid patriotic effort. You are well on your way to becoming a true reformist. Keep studying the logs of our heroes.",
      ...
      return {
        ...
        description:
          "You flew too close to the attractive flame! Do Doña Teodora proud and study the archives again to protect your wings.",
    ...
    <div className="space-y-12 animate-fade-in" id="quiz-view-container">
      <div className="text-center space-y-2">
        ...
        <p className="font-serif italic text-xs md:text-sm text-gold-800/80 max-w-2xl mx-auto">
          "Test your knowledge of the polymath of Calamba. A random selection of
          archival questions to prove your Ilustrado credentials."
        </p>
        <div className="w-20 h-0.5 bg-crimson-700 mx-auto mt-3 mb-4" />
        <div className="flex justify-center" id="audio-toggle-container">
          <button
            onClick={handleToggleMute}
            className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#fdfaf4] hover:bg-gold-50 border border-gold-400/40 hover:border-gold-500 shadow-xs transition-all duration-300 text-[10px] md:text-xs text-crimson-900 font-display font-bold tracking-widest cursor-pointer select-none"
            title={isMuted ? "Unmute Music & FX" : "Mute Music & FX"}
            id="audio-toggle-btn"
          >
            {isMuted ? (
              <>
                <VolumeX size={14} className="text-amber-950/60" />
                <span>SOUNDS: MUTED</span>
              </>
            ) : (
              <>
                <Volume2 size={14} className="text-emerald-700 animate-pulse" />
      ...
      {questions.length > 0 && !quizFinished && (
        <div className="max-w-3xl mx-auto" id="quiz-playing-stage">
          <div
            className="parchment-card p-6 md:p-10 rounded-sm border-2 border-double border-gold-500 relative overflow-hidden"
            id="quiz-box-card"
          >
            ...
            <div
              className="flex justify-between items-center border-b border-gold-300/30 pb-3 mb-6"
              id="quiz-tracker"
            >
              ...
              <div className="flex items-center gap-1.5 text-xs font-serif italic text-gold-800">
                ...
                Current Category:{" "}
                <strong className="text-crimson-950 ml-0.5">
            ...
            {/* Question prompt */}
            <h3
              className="font-serif text-lg md:text-xl font-bold text-crimson-950 leading-snug mb-6"
            ...
            <div className="grid grid-cols-1 gap-3.5" id="quiz-options">
              {activeQuestion.options.map((option, index) => {
                ...
                if (selectedOptionIdx === index) {
                  optionStyle =
                    "bg-gold-100 border-gold-500 ring-1 ring-gold-500 font-semibold";
                }
                ...
                if (isSubmitted) {
                  if (index === activeQuestion.correctAnswerIndex) {
                    ...
                    optionStyle =
                      "bg-faf6ee text-amber-950/40 border-gold-300/20";
                  }
                }
            ...
            {/* Feedback & Actions */}
            <div
              className="mt-8 pt-6 border-t border-gold-300/25 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
              id="quiz-actions"
            >
              <div className="flex-grow">
                {isSubmitted && (
                  <div
                    className="p-4 bg-gold-50 border border-dashed border-gold-600/20 rounded-xs space-y-1.5 text-xs text-justify animate-fade-in"
                    id="explanation-box"
                  >
      ...
      {quizFinished && (
        <div className="max-w-3xl mx-auto space-y-8" id="quiz-results-stage">
          ...
          <div
            className="parchment-card p-6 md:p-8 rounded-sm border border-gold-400 space-y-4 text-center"
          ...
          >
            ...
            <div
              ...
              id="results-score-row"
            >
              <span className="font-serif text-5xl font-extrabold text-crimson-950">
                {score}
              </span>
            ...
            <div
              className={`p-4 border rounded-sm ${getEvaluation(score).color} max-w-xl mx-auto space-y-1.5`}
              id="results-grade-box"
            >
          ...
          <div
            className="bg-faf6ee border border-gold-400/50 p-6 md:p-8 rounded-sm space-y-6"
            id="certificate-generator-panel"
          >
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-amber-900/60 font-serif italic">
                Input your name below to stamp your credential into the
                Scholastica Archives.
              </p>
            </div>
            ...
            {!certificateIssued ? (
              ...
              >
                ...
                <button
                  onClick={() =>
                    studentName.trim() && setCertificateIssued(true)
                  }
                  disabled={!studentName.trim()}
                  className={`px-5 py-2 rounded-sm font-display font-bold text-xs tracking-wider transition-all duration-200 border ${studentName.trim() ? "bg-crimson-800 text-gold-100 border-crimson-950 cursor-pointer hover:bg-crimson-950" : "bg-gold-50/40 text-amber-900/40 border-gold-300/20 cursor-not-allowed"}`}
                  id="issue-cert-btn"
                >
                  ISSUE CERTIFICATE
                </button>
              ...
              <div
                ...
                id="certificate-exhibition"
              >
                {/* Gilded Certificate Layout */}
                <div
                  className="p-8 md:p-12 border-8 border-double border-gold-500 bg-white text-amber-950 relative overflow-hidden flex flex-col items-center shadow-2xl text-center"
                ...
                >
                  ...
                  <p className="font-serif text-xs leading-relaxed max-w-lg text-amber-950/80 mb-6">
                    for demonstrating high academic competence and historical
                    memory by completing the competitive examination on the
                    life, polymathic masterpieces, family genealogy, and
                  ...
                  <div
                    className="grid grid-cols-2 gap-8 w-full max-w-md pt-4 border-t border-gold-600/20"
                    id="cert-signatories"
                  >
                    <div className="space-y-1">
                      <p className="font-serif font-bold text-[10px] text-crimson-950 uppercase tracking-widest">
                        {score} / 5
                      </p>
                      <div className="w-16 h-px bg-gold-400 mx-auto" />
                      <p className="font-serif italic text-[8px] text-gold-700">
                        Official Exam Score
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="font-mono text-[9px] text-crimson-950">
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.615 0-1.12-.511-1.12-1.227L6.34 18m11.318-18h-11.32"
                      />
                    </svg>
                    PRINT CREDENTIAL
                  </button>
